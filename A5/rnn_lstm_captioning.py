...
def rnn_step_backward(dnext_h, cache):
    ...
    # dLdu = dLdnext_h * dnext_h*du = dnext_h * (dtanh(u)/du)
    dLdu = (1 - u * u) * dnext_h # dtanh(x) = 1 - tanh^2(x) in (N, H)
    
    dx = dLdu @ Wx.T # (N, H) * (H, D) -> (N, D)

    dprev_h = dLdu @ Wh.T #(N, H) * (H, H) -> (N, H)

    dWx = (x.T) @ dLdu # (D, N) * (N, H) -> (D, H)

    dWh = prev_h.T @ dLdu # (H, N) * (N, H) -> (H, H)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h = torch.zeros(T, N, H).to(x) # (T, N, H)
    prev_h = h0
    cache = []
    for i in range(T):
...
def rnn_backward(dh, cache):
    ...
    N, T, H = dh.shape
    D = cache[0][0].shape[1]
    dH = dh.permute(1, 0, 2) # (T, N, H)

    dx = torch.zeros(T, N, D).to(dh) # (T, N, D)
    dh0 = torch.zeros(N, H).to(dh) # (N, H)
    dWx = torch.zeros(D, H).to(dh) # (D, H)
    dWh = torch.zeros(H, H).to(dh) # (H, H)
    db = torch.zeros(H,).to(dh) # (H,)

    for i in reversed(range(T)):
        backward_out = rnn_step_backward(dh0 + dH[i], cache[i])
        dx[i] = backward_out[0]
        dh0 = backward_out[1] # dprev_h
        dWx += backward_out[2]
        dWh += backward_out[3]
        db += backward_out[4]

    dx = dx.permute(1, 0, 2) # (N, T, D)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        if cell_type in {"rnn", "lstm"}:
            self.feature_proj = nn.Sequential(nn.AvgPool2d(4, 4), # N, D, 1, 1
                                              nn.Flatten(),       # N, D
        ...
        # caption model
        if cell_type is "rnn":
            self.caption_model = RNN(wordvec_dim, hidden_dim)
        elif cell_type is "lstm":
            self.caption_model = LSTM(wordvec_dim, hidden_dim)
        elif cell_type is "attn":
            self.caption_model = AttentionLSTM(wordvec_dim, hidden_dim)

        # project from hidden state to vocab probability
    ...
    def forward(self, images, captions):
        ...
        ######################################################################
        # Step 1: get the image features from the backbone network
        ...
        #         work embedding & h0
        hidden_states = self.caption_model(embedded, h0) # N, T, H
        
        # Step 4: use a simple fully-connected network to project from hidden
        #         state to vocabulary space
        voc = self.output_proj(hidden_states) # N, T, V
    ...
    def sample(self, images, max_length=15):
        ...
        for i in range(max_length):
            # Step 1: Embed the previous word using the learned word embeddings
            embedded = self.embedding(prev_words) # N, W

            # Step 2: Make an RNN step using the previous hidden state and the 
            # embedded word to get the next hidden state.
            if self.cell_type is "rnn":
                prev_h = self.caption_model.step_forward(embedded, prev_h) # (N, H)
            elif self.cell_type is "lstm":
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        activation = x @ self.Wx + prev_h @ self.Wh + self.b # N, 4H
        ai, af, ao, ag = activation.chunk(4, dim=1)
        i = torch.sigmoid(ai) # N, H
        f = torch.sigmoid(af) # N, H
        o = torch.sigmoid(ao) # N, H
        g = torch.tanh(ag)    # N, H
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        H = h0.shape[1]
        X = x.permute(1, 0, 2) # T, N, D
        hn = torch.zeros(T, N, H).to(h0) # T, N, H
...
def dot_product_attention(prev_h, A):
    ...
    ##########################################################################
    A_flatten = A.view(N, H, 16) #(N, H, 16)

    # (N, 1, H) @ (N, H, 16) -> (N, 1, 16)
    attn_weights = prev_h.unsqueeze(1) @ A_flatten / math.sqrt(H)
    attn_weights = attn_weights.permute(0, 2, 1) # (N, 16, 1)
    attn_weights = torch.softmax(attn_weights, dim = 1) # (N, 16, 1)
    
    attn = A_flatten @ attn_weights # (N, H, 16) @ (N, 16, 1) -> (N, H, 1)
    attn = attn.squeeze() # (N, H)
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        activation = x @ self.Wx + prev_h @ self.Wh + attn @ self.Wattn + self.b # N, 4H
        ai, af, ao, ag = activation.chunk(4, dim=1)
        i = torch.sigmoid(ai) # N, H
        f = torch.sigmoid(af) # N, H
        o = torch.sigmoid(ao) # N, H
