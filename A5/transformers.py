...
def generate_token_dict(vocab):
    ...
    ##############################################################################
    token_dict = dict(zip(vocab, range(len(vocab))))
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return token_dict
...
def scaled_dot_product_two_loop_single(
    ...
    # output of the dot product will then be scaled by dividing it with sqrt(M)   #
    ...
    K, M = query.shape
    K_key = key.shape[0]
    scaled_weights = torch.empty(K_key).to(query)
    for i in range(K):
        for j in range(K_key):
            scaled_weight = query[i].dot(key[j]) / math.sqrt(M)
            scaled_weights[j] = scaled_weight
        scaled_weights = torch.softmax(scaled_weights, dim=0)  # K_key
        out[i] = scaled_weights @ value  # M
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_batch(
    ...
    for i in range(N):
        for j in range(K):
            q = query[i, j].unsqueeze(dim=-1)  # (M, 1)
            scaled_weights = key[i] @ q / math.sqrt(M) # (K, M)@(M, 1) -> (K, 1)
            scaled_weights = torch.softmax(scaled_weights, dim=0).squeeze() # (K)
            out[i, j] = scaled_weights @ value[i]
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        ##########################################################################
        cq = math.sqrt(6 / (dim_in + dim_q))
        cv = math.sqrt(6 / (dim_in + dim_v))

        self.q = nn.Linear(dim_in, dim_q)
        nn.init.uniform_(self.q.weight, a = -cq, b = cq)

        self.k = nn.Linear(dim_in, dim_q)
        nn.init.uniform_(self.k.weight, a = -cq, b = cq)

        self.v = nn.Linear(dim_in, dim_v)
        nn.init.uniform_(self.v.weight, a = -cv, b = cv)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        for i in range(num_heads):
            self.selfAttentions.append(SelfAttention(dim_in, dim_out, dim_out))

        self.back = nn.Linear(dim_out * num_heads, dim_in)
        c = math.sqrt(6 / (dim_out * num_heads + dim_in))
        nn.init.uniform_(self.back.weight, a=-c, b=c)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        args:
            ...
            value: Tensor of shape (N, K, V) where N is the number of sequences in
                the batch, K is the sequence length and M is the input embedding
                dimension. M should be equal to dim_in in the init function
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        mlp_one = nn.Linear(inp_dim, hidden_dim_feedforward)
        nn.init.uniform_(mlp_one.weight, a=-c, b=c)

        mlp_two = nn.Linear(hidden_dim_feedforward, inp_dim)
        nn.init.uniform_(mlp_two.weight, a=-c, b=c)

        self.MLPs = nn.Sequential(mlp_one,
                                  nn.ReLU(),
                                  mlp_two)
        ##########################################################################
        #               END OF YOUR CODE                                         #
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        ##########################################################################
        self.attention = MultiHeadAttention(num_heads, emb_dim, int(emb_dim/num_heads))
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        """
        args:
            dec_inp: a Tensor of shape (N, K2, M)
            enc_inp: a Tensor of shape (N, K1, M)
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    i = torch.tensor(list(range(M))).unsqueeze(0)  # (1, M)
    a = torch.floor(i / M) # (1, M)
    p = torch.tensor(list(range(K))).unsqueeze(-1)  # (K, 1)
